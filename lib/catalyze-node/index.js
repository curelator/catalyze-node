...
Catalyze.prototype = {
  ...
  createUser: function (data, callback) {
    ...
    // workaround extras creation bug (cannot add extras on the POST /v1/user route)
    var wrapper_cb = function (err, res, body, success) {
      callback(err, res, body, success);
    }
    
    if (_.has(data, "extras")) {
      
      var extra_data = {
        "extras":data.extras
      }
      
      wrapper_cb = function (err, res, body, success) {
        if (success) {
          _this.sessionToken = body.sessionToken;
          _this.updateUser(extra_data, callback);
        }
        else {
          callback(err, res, body, success);
        }
    ...
    this._jsonRequest({
      ...
      callback: function (err, res, body, success) {
        ...
        wrapper_cb(err, res, body, success);
      }
    });
  },
